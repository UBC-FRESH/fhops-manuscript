...
Primary modules are \texttt{operational.py} and \texttt{data.py} under
\breakpath{src/fhops/model/milp}. Primary regression tests live under
\breakpath{tests/model} and cover the MILP builder plus driver integration.
...
\hline
\textbf{OBJ} & \texttt{model.objective}; weights:
...
\textbf{E8} & \texttt{model.activation\_prod}, \texttt{model.head\_start},
\texttt{model.role\_active\_upper}, \texttt{model.role\_active\_lower} \\
\textbf{E9} & \texttt{model.loader\_batch}, \texttt{model.loader\_partial\_cap} \\
\textbf{E10} & \texttt{model.block\_balance}
(\texttt{block\_balance\_rule}) plus \texttt{model.leftover} \\
\textbf{E11} & \texttt{model.landing\_capacity}
(\texttt{landing\_capacity\_rule}) plus \texttt{model.landing\_surplus} \\
\textbf{D1} & Domain declarations for \texttt{model.x}, \texttt{model.y},
\texttt{model.role\_active}, \texttt{model.loads}, \texttt{model.prod},
\texttt{model.role\_prod}, \texttt{model.inventory\_start},
\texttt{model.inventory}, \texttt{model.loader\_partial},
\texttt{model.leftover}, and \texttt{model.landing\_surplus} \\
\hline
\end{tabular}
\end{center}
